...
string 				_modInfoMsgPrefix 		= ""		; Msg prefix for backup messages (X/XX)
...
; YeOlde
function RestorePages(int jMod, string modInfoMsgPrefix)
	Log("RestorePages() -> " + modInfoMsgPrefix)
	_modInfoMsgPrefix = modInfoMsgPrefix
	; string[] dataArray = Utility.CreateStringArray(1, "")
	; if (JMap.hasKey(jMod, "data"))
	; 	int jData = JMap.getObj(jMod, "data")
	; 	dataArray = JArray.asStringArray(jData)
	; endif
...
function OnRestoreRequest(int jMod)
	...
	if (jPage > 0)
		...
		_configManager.ShowBackupInfoMsg(_modInfoMsgPrefix + ": mod '" + ModName + "', page '(none)'...")
		ImportPageOptions(jPage)		
		_configManager.ShowBackupInfoMsg(_modInfoMsgPrefix + ": mod '" + ModName + "', page '(none)'... (DONE)")
	endif
	...
	while (i < Pages.Length)
		...
		if (jPage > 0)
			string msg = _modInfoMsgPrefix + ": mod '" + ModName + "', page '" + Pages[i] + "'..."
...
string _backbackupDirectory
; YeOlde
function BackupAllPagesOptions(int jBackup, string backupDirectory, string modInfoMsgPrefix)	
	Log("BackupAllPagesOptions() -> " + ModName)
	_modInfoMsgPrefix = modInfoMsgPrefix
	_backbackupDirectory = backupDirectory
	; Create/Get the mod config handler
	int jMod = BackupConfig.AddMod(jBackup, ModName)
	string[] strData = OnBackupRequest(jMod)	
	if (strData != none && !(strData.Length == 1 && strData[0] == ""))
		int jData = JArray.objectWithStrings(strData)
		JMap.setObj(jMod, "data", jData)
	endif
	Log("BackupAllPagesOptions() -> SAVE FILE")
	JValue.writeToFile(jMod, backupDirectory + ModName + ".json")
