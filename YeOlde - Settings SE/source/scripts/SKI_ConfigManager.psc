...
function GenerateDefaultBlackList()
	...
	JArray.addStr(_jBlackList, "A Matter of Time")
	JArray.addStr(_jBlackList, "AH Hotkeys")
	JArray.addStr(_jBlackList, "Campfire") ; Mod backup itself automatically
	JArray.addStr(_jBlackList, "Complete Alchemy") ; Patch available
	JArray.addStr(_jBlackList, "Frostfall") ; Mod backup itself automatically
	JArray.addStr(_jBlackList, "Hunterborn")
	JArray.addStr(_jBlackList, "iEquip") ; Patch available
	JArray.addStr(_jBlackList, "Skyrim Unbound")
	JArray.addStr(_jBlackList, "Skyrim Wayshrines")
	JArray.addStr(_jBlackList, "Wearable Lanterns") ; Mod backup itself automatically
...
bool function IsModBlackListed(string mod)
	bool result = !(JArray.findStr(_jBlackList, mod) == -1)
	Log("IsPageBackupBlackListed(" + mod + ") -> result:" + result)
	return result
endfunction
...
function BackupAllModValues(yeolde_mcm_settings settings_mod)
	...
	YeOldeBackupInfo.addStrIndexedArray(_jBackupInfo, "3- backup mod starting order", _allNames)
	...
	while (i<_allMods.Length && nbModsDone < nbMods)
		...
		if(_allMods[i] != none)
			...
			if(IsModBlackListed(modName))
				...
				settings_mod.UpdateInfoMsg(msgPrefix + "Mod '" + modName + "' is on your blacklist. Mod skipped.")
			else
				Log("YeOlde_JSON:ModName -> " + modName)					
				config = _allMods[i]
				config.BackupAllPagesOptions(jBackup, BackupConfig.GetDefaultBackupModDirectory(), msgPrefix)
...
; YeOlde
function RestoreMcmValues(yeolde_mcm_settings settings_mod)	
	Log("RestoreMcmValues()")
	SKI_ConfigBase config
	...
	if(jBackup == 0)
		settings_mod.UpdateInfoMsg("ERROR: Backup file not found")
		Log("RestoreMcmValues() -> ERROR: Backup file not found")
		return
	endif

	Log("RestoreMcmValues() -> allKeysPArray()")
	string[] mods = JMap.allKeysPArray(jBackup)
	Log("RestoreMcmValues() -> nbMods: " + mods.length)

	int i = 0
	while (i< mods.length)
		string msgPrefix = "(" + (i+1) + "/" + mods.length + ")"
		settings_mod.UpdateInfoMsg("Mod " + msgPrefix + "... ")	
		...
		settings_mod.UpdateInfoMsg("Mod " + msgPrefix + "... DONE")	
		i += 1
	endWhile

	; Input.TapKey(15) ; press TAB to exit current menu
	
	Log("RestoreMcmValues -> RESTORATION COMPLETED")
